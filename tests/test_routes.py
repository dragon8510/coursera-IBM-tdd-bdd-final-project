...
class TestProductRoutes(TestCase):
    ...
    def test_query_product_list_by_availability(self):
        ...
        availability_products = [product for product in products if product.available == test_availability]
        response = self.client.get(BASE_URL, query_string=f"available={test_availability}")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        data = response.get_json()
        self.assertEqual(len(data), len(availability_products))
        # check the data just to be sure
        for product in data:
